...
@MyBot.dp.callback_query_handler(posts_cb.filter(action=['bagration_user_get_data']))
async def call_bagration_user_get_data(call: types.CallbackQuery = None, callback_data: dict = None,
    ...
    if not get_sett(cat='enable_features', param='use_bagration_user_get_data').get_set():
