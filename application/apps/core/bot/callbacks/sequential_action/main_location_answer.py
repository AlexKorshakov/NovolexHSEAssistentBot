...
from apps.core.bot.filters.custom_filters import filter_main_location
...
from apps.core.bot.callbacks.sequential_action.data_answer import get_and_send_main_location_data
...
@MyBot.dp.callback_query_handler(filter_main_location, state=ViolationData.all_states)
async def main_location_answer(call: types.CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        main_location_id = datas_query[0][0] if datas_query else None

        await set_violation_atr_data("main_location", art_val=call.data, state=state)
        await set_violation_atr_data("main_location_id", main_location_id, state=state)
